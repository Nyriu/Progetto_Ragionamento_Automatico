...
time_sorted_data %>%
  ...
  geom_line() +
  geom_point(mzn_timeouted, mapping=aes(x=sorted_num, y=rep(TIMEOUT_time +  5,nrow(mzn_timeouted)), color=model_type), show.legend = F) +
  geom_point(lp_timeouted,  mapping=aes(x=sorted_num, y=rep(TIMEOUT_time + 10,nrow(lp_timeouted )), color=model_type), show.legend = F) +
