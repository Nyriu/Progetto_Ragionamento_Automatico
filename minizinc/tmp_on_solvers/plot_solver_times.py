# TODO
# Plotta informazioni sui solveTime al variare dei solver
...
def get_all_data(solver_dir, not_filter=False):
    """ Raccolgo i dati che devo plottare """
    if not solver_dir[-1] == '/':
        solver_dir += '/'

    inputs_dir = solver_dir + 'inputs/'
    ...
    num = 0
    inp = my_lib.get_input(num, inputs_dir)
    out = my_lib.read_output(num, outputs_dir, suppress_error=True)
    while not (out == None):
        if (inp['K'] == K and inp['H'] == H) or not_filter:
            inputs.append(inp)
...
def double_plot(s_inputs, s_nums, s_solveTimes1, s_solveTimes2, show=False, save=True, solvers_labels=[""]):
    ...
    # Plotto tempi
    times1 = ax1.plot(x, s_solveTimes1, color='red')
    times2 = ax1.plot(x, s_solveTimes2, color='blue')
    # End # Plotto tempi
    ...
    lns = [rects_m, rects_p, rects_o, rects_q, times1[0], times2[0]]
    labs = [l.get_label() for l in lns]
    labs = labs[:-2] + ['solveTimes '+solvers_labels[0], 'solveTimes2 '+solvers_labels[1]]
    ax.legend(lns, labs)
...
def main():
    ...
    g_inputs, g_solveTimes, g_nums = get_all_data(gecode_dir , not_filter=True)
    c_inputs, c_solveTimes, c_nums = get_all_data(chuffed_dir, not_filter=True)

    cut_len = min(len(g_inputs), len(c_inputs))

    g_inputs, g_solveTimes, g_nums = g_inputs[:cut_len], g_solveTimes[:cut_len], g_nums[:cut_len]
    c_inputs, c_solveTimes, c_nums = c_inputs[:cut_len], c_solveTimes[:cut_len], c_nums[:cut_len]
    ...
    inputs = g_inputs
    nums   = g_nums
    solveTimes1 = g_solveTimes
    solveTimes2 = c_solveTimes
    solvers_labels = ['gecode', 'chuffed']

    double_plot(inputs, nums, solveTimes1, solveTimes2, show, save, solvers_labels)
